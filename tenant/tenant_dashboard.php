...
<body class="antialiased flex h-screen overflow-hidden">
    ...
    <main class="main-content flex-1 p-10 overflow-y-auto no-scrollbar">
        ...
        <header class="mb-10 flex justify-between items-end">
            <div>
                <h2 class="text-3xl font-black uppercase tracking-tighter text-white italic">Welcome Back, <span
                        class="text-primary italic"><?= htmlspecialchars($first_name) ?></span></h2>
                <p class="text-gray-500 text-[10px] font-black uppercase tracking-widest mt-1 italic">
                    <?= htmlspecialchars($gym_name) ?> Management System</p>
            </div>
        ...
        <?php if (strpos($sub_status, 'Pending Approval') !== false): ?>
            <div class="glass-card p-6 border-amber-500/30 bg-amber-500/5 mb-8 flex items-center gap-6">
                ...
                <div class="flex-1">
                    <h4 class="text-sm font-black uppercase italic tracking-tight text-amber-400">Subscription Pending
                        Approval</h4>
                    <p class="text-[10px] font-bold text-amber-500/70 uppercase tracking-widest mt-1">Your payment is being
                        verified by our team. Access to some features might be restricted until your plan is activated.</p>
                </div>
                <div class="text-right">
                    <span
                        class="px-3 py-1 rounded-full bg-amber-500/20 text-amber-400 text-[9px] font-black uppercase tracking-widest border border-amber-500/30">Verification
                        In Progress</span>
                </div>
            </div>
        <?php elseif ($sub_status === 'None' || $sub_status === 'Expired' || $sub_status === 'Inactive' || strpos($sub_status, 'Suspended') !== false): ?>
            <div class="glass-card p-6 border-rose-500/30 bg-rose-500/5 mb-8 flex items-center gap-6">
                ...
                <div class="flex-1">
                    <h4 class="text-sm font-black uppercase italic tracking-tight text-rose-400">
                        <?= $is_suspended ? 'System Access Suspended' : 'No Active Subscription' ?></h4>
                    <p class="text-[10px] font-bold text-rose-500/70 uppercase tracking-widest mt-1">Activate a growth plan
                        to unlock the full potential of your gym's digital infrastructure.</p>
                </div>
                <a href="subscription_plan.php"
                    class="h-10 px-6 rounded-xl bg-rose-500 text-white text-[10px] font-black uppercase tracking-widest flex items-center justify-center hover:opacity-90 transition-all">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            <div class="glass-card p-6 flex items-center gap-5 relative overflow-hidden group">
                <div
                    class="size-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary shrink-0 group-hover:scale-110 transition-transform">
            ...
            <div class="glass-card p-6 flex items-center gap-5 relative overflow-hidden group">
                <div
                    class="size-12 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 shrink-0 group-hover:scale-110 transition-transform">
            ...
            <div class="glass-card p-6 flex items-center gap-5 relative overflow-hidden group">
                <div
                    class="size-12 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500 shrink-0 group-hover:scale-110 transition-transform">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-extrabold mt-1 tracking-tight italic uppercase">
                        ₱<?= number_format($monthly_rev, 0) ?></h3>
                </div>
            ...
            <div class="glass-card p-6 flex items-center gap-5 relative overflow-hidden group">
                <div
                    class="size-12 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-500 shrink-0 group-hover:scale-110 transition-transform">
                ...
                <div>
                    <p class="text-[10px] font-bold uppercase text-gray-500 tracking-widest flex items-center gap-1">
                        ...
                        <?php if ($payment_term === 'Monthly'): ?><span
                                class="px-1.5 py-0.5 rounded-sm bg-blue-500/20 text-blue-400 text-[7px] font-black">1/Mo</span><?php endif; ?>
                    </p>
                    <div class="flex items-center gap-2 mt-1">
                        <h3
                            class="text-xl font-extrabold tracking-tight uppercase <?= ($is_sub_active && strtolower($sub_status) === 'active') ? 'text-emerald-500' : 'text-rose-500' ?> italic leading-none">
                            <?= htmlspecialchars($sub_status) ?></h3>
                    </div>
                    <?php if ($billing_label): ?>
                        <p class="text-[9px] font-black uppercase tracking-widest mt-1 <?= $billing_status_color ?>">
                            <?= $billing_label ?></p>
                    <?php endif; ?>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
            <div class="glass-card p-6">
                ...
                <div class="p-6 rounded-3xl bg-background-dark/50 border border-white/5 mb-6">
                    <div class="flex items-center gap-4">
                        <div
                            class="size-12 rounded-xl bg-[#1a1821] flex items-center justify-center overflow-hidden shadow-inner">
                            <?php if (!empty($page['logo_path'])): ?>
                        ...
                        <div>
                            <h5 class="text-sm font-bold italic uppercase tracking-tight">
                                <?= htmlspecialchars($gym_name) ?></h5>
                            <div class="flex items-center gap-2 mt-1.5">
                                <div class="size-3.5 rounded-full border border-white/20"
                                    style="background-color: <?= $theme_color ?>"></div>
                                <p class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Theme Accent
                                    Color</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex gap-4">
                    <a href="tenant_settings.php"
                        class="flex-1 h-12 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all flex items-center justify-center text-[10px] font-bold uppercase tracking-widest gap-2">
                        <span class="material-symbols-outlined text-lg">edit_note</span> Edit Page
                    </a>
                    <a <?= $is_sub_active ? 'target="_blank" href="../portal.php?gym=' . htmlspecialchars($page['page_slug'] ?? '') . '"' : 'onclick="showSubWarning()"' ?>
                        class="flex-1 h-12 rounded-xl <?= $is_sub_active ? 'bg-primary shadow-primary/20 hover:opacity-90' : 'bg-white/5 border border-white/10 text-gray-500 hover:text-white' ?> transition-all flex items-center justify-center text-[10px] font-bold uppercase tracking-widest gap-2 text-white shadow-lg cursor-pointer">
                        <span
                            class="material-symbols-outlined text-lg"><?= $is_sub_active ? 'visibility' : 'lock' ?></span>
            ...
            <div class="glass-card p-6">
                ...
                <div class="grid grid-cols-1 gap-3">
                    <a href="profile.php"
                        class="group p-5 rounded-3xl bg-white/5 border border-white/5 hover:border-indigo-500/50 transition-all flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            <div
                                class="size-12 rounded-2xl bg-indigo-500/10 flex items-center justify-center text-indigo-500 group-hover:scale-110 transition-transform">
                                <span class="material-symbols-outlined text-xl">account_circle</span>
                            </div>
                            <div>
                                <p class="text-xs font-bold uppercase italic">My Profile</p>
                                <p class="text-[9px] text-gray-600 uppercase font-bold mt-0.5">Edit personal and account
                                    details</p>
                            </div>
                        </div>
                        <span
                            class="material-symbols-outlined text-gray-600 group-hover:translate-x-1 transition-transform">chevron_right</span>
                    </a>

                    <a href="staff.php"
                        class="group p-5 rounded-3xl bg-white/5 border border-white/5 hover:border-primary/50 transition-all flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            <div
                                class="size-12 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-500 group-hover:scale-110 transition-transform">
                                <span class="material-symbols-outlined text-xl">person_add</span>
                            </div>
                            <div>
                                <p class="text-xs font-bold uppercase italic">Add Team Staff</p>
                                <p class="text-[9px] text-gray-600 uppercase font-bold mt-0.5">Manage employees and
                                    roles</p>
                            </div>
                        </div>
                        <span
                    ...
                    <a href="my_users.php"
                        class="group p-5 rounded-3xl bg-white/5 border border-white/5 hover:border-emerald-500/50 transition-all flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            <div
                                class="size-12 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 group-hover:scale-110 transition-transform">
                                <span class="material-symbols-outlined text-xl">group_add</span>
                            </div>
                            <div>
                                <p class="text-xs font-bold uppercase italic">Member Directory</p>
                                <p class="text-[9px] text-gray-600 uppercase font-bold mt-0.5">Manage active memberships
                                </p>
                            </div>
                        </div>
                        <span
    ...
    <script>
        ...
        new Chart(document.getElementById('revenueChart'), {
            type: 'line',
            data: {
                labels: [<?php foreach ($revenue_trends as $r)
                    echo "'" . $r['month'] . "',"; ?>],
                datasets: [{
                    label: 'Revenue',
                    data: [<?php foreach ($revenue_trends as $r)
                        echo $r['amount'] . ","; ?>],
        ...
        new Chart(document.getElementById('growthChart'), {
            type: 'bar',
            data: {
                labels: [<?php foreach ($member_growth as $m)
                    echo "'" . $m['month'] . "',"; ?>],
                datasets: [{
                    label: 'New Members',
                    data: [<?php foreach ($member_growth as $m)
                        echo $m['count'] . ","; ?>],
        ...
        <div
            ...
            <?php if (!$is_suspended): ?>
                <button onclick="closeSubModal()"
                    class="absolute top-6 right-6 text-gray-400 hover:text-white transition-all size-10 rounded-xl hover:bg-white/5 flex items-center justify-center">
                    <span class="material-symbols-outlined">close</span>
                </button>
            <?php endif; ?>

            <div
                class="size-20 rounded-3xl <?= $is_suspended ? 'bg-red-500/10 border-red-500/20' : 'bg-primary/10 border-primary/20' ?> border flex items-center justify-center mx-auto mb-8">
                <span
                    class="material-symbols-outlined text-4xl <?= $is_suspended ? 'text-red-500' : 'text-primary' ?>">lock</span>
            </div>
            <h3 class="text-2xl font-black italic uppercase tracking-tighter text-white mb-3">
                <?= $is_suspended ? 'System Suspended' : 'Subscription Required' ?></h3>
            <p
                ...
                <br><br>Status: <span
                    class="<?= $is_suspended ? 'text-red-500' : 'text-primary' ?> italic animate-pulse"><?= $sub_status ?></span>
            </p>
            <div class="flex flex-col gap-4">
                <a href="subscription_plan.php"
                    class="h-14 rounded-2xl <?= $is_suspended ? 'bg-red-600 shadow-red-500/20' : 'bg-primary shadow-primary/20' ?> text-white text-[11px] font-black uppercase italic tracking-[0.2em] flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-95 transition-all shadow-xl group">
                    <span
                ...
                <?php if (!$is_suspended): ?>
                    <button onclick="closeSubModal()"
                        class="h-14 rounded-2xl bg-white/5 border border-white/10 text-gray-400 text-[11px] font-black uppercase italic tracking-[0.2em] flex items-center justify-center hover:bg-white/10 transition-all">
                        Dismiss
                    </button>
                <?php endif; ?>
            </div>
