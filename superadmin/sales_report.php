...
<head>
    ...
    <style>
        ...
        * {
        ...
        .tab-btn { position: relative; transition: all 0.3s ease; color: var(--text-main); opacity: 0.6; cursor: pointer; border: none; outline: none; background: none; } 
        .tab-btn.active { color: var(--primary); opacity: 1; } 
        .tab-indicator { position: absolute; bottom: 0; left: 0; right: 0; height: 2px; background-color: var(--primary); border-radius: 9999px; transition: all 0.3s ease; opacity: 0; pointer-events: none; }
    ...
    <main class="flex-1 p-6 md:p-10 max-w-[1400px] w-full mx-auto">
        <header class="mb-12 flex flex-row justify-between items-end gap-6">
            <div>
                ...
                <p class="text-[--text-main] text-[10px] font-bold uppercase tracking-widest mt-2 px-1 opacity-80">Revenue & Transaction Tracking</p>
            </div>
            <div class="flex items-end gap-8 text-right shrink-0">
                <div class="flex flex-col items-end">
                    <p id="headerClock" class="text-[--text-main] font-black italic text-2xl leading-none tracking-tighter uppercase transition-colors cursor-default">00:00:00 AM</p>
        ...
        <div class="glass-card mb-8 p-8">
            <form method="GET" class="flex flex-wrap items-end gap-6">
                ...
                <div class="space-y-2">
                    <p class="text-[10px] font-black uppercase tracking-widest text-[--text-main]/60 ml-1">Start Date</p>
                    <input type="date" name="date_from" value="<?= $date_from ?>" class="input-field">
                </div>
                <div class="space-y-2">
                    <p class="text-[10px] font-black uppercase tracking-widest text-[--text-main]/60 ml-1">End Date</p>
                    <input type="date" name="date_to" value="<?= $date_to ?>" class="input-field">
                </div>
                <div class="flex items-center gap-4">
                    <button type="submit" class="h-[46px] px-8 rounded-xl bg-primary text-white text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-transform active:scale-95 shadow-lg shadow-primary/20">Update Report</button>
                    <a href="sales_report.php" class="text-[10px] font-black uppercase tracking-widest text-[--text-main]/60 hover:text-white transition-colors">Clear Filters</a>
                </div>
            </form>
        </div>
        ...
        <div id="overviewTab" class="tab-content <?= ($active_tab == 'overviewTab') ? 'active' : '' ?>">
            ...
            <div class="glass-card p-8 relative overflow-hidden group">
                ...
                <p class="text-[10px] font-black uppercase text-[--text-main] opacity-60 tracking-widest mb-2">Total Revenue</p>
                <h2 class="text-3xl font-black text-[--text-main] italic mt-2">₱<?= number_format($total_revenue, 2) ?></h2>
            ...
            <div class="glass-card p-8 relative overflow-hidden group">
                ...
                <p class="text-[10px] font-black uppercase text-[--text-main] opacity-60 tracking-widest mb-2">Top Tenant</p>
                <h2 class="text-2xl font-black text-[--text-main] italic mt-2"><?= $tenant_sales[0]['gym_name'] ?? 'N/A' ?></h2>
            ...
            <div class="glass-card p-8 flex flex-col">
                ...
                <div class="space-y-4 overflow-y-auto no-scrollbar max-h-[400px]">
                    <?php if (empty($tenant_sales)): ?>
                        <p class="text-xs text-[--text-main]/40 italic font-bold text-center mt-10 uppercase">No sales recorded</p>
                    <?php else: ?>
                        ...
                        <div class="p-4 rounded-2xl bg-white/[0.02] border border-white/5 flex justify-between items-center hover:bg-white/[0.05] transition-colors">
                            <div>
                                <p class="text-sm font-bold text-[--text-main]"><?= htmlspecialchars($ts['gym_name']) ?></p>
                                <p class="text-[9px] text-[--text-main]/40 font-black uppercase tracking-widest italic"><?= $ts['transaction_count'] ?> sales</p>
                            </div>
        ...
        <div class="glass-card overflow-hidden">
            <div class="px-8 py-6 border-b border-white/5 bg-white/[0.01] flex justify-between items-center">
                <div>
                    <h3 class="text-sm font-black italic uppercase tracking-widest text-primary">Transaction History Summary</h3>
                    <p class="text-[9px] text-[--text-main]/40 font-bold uppercase tracking-widest mt-1">Detailed logs of all successful payments</p>
                </div>
                <div class="flex items-center gap-3">
                    <button onclick="exportReportToPDF(true)" class="px-5 py-2.5 rounded-xl bg-white/5 border border-white/5 text-[10px] font-black uppercase tracking-widest text-[--text-main]/60 hover:text-white hover:bg-white/10 transition-all flex items-center gap-2">
                        <span class="material-symbols-outlined text-sm">visibility</span>
                        Preview
                    </button>
                    <button onclick="exportReportToPDF(false)" class="px-5 py-2.5 rounded-xl bg-primary text-white text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-transform active:scale-95 shadow-lg shadow-primary/20 flex items-center gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-background/50 text-[--text-main]/60 text-[10px] font-black uppercase tracking-widest">
                    ...
                    <tbody class="divide-y divide-white/5">
                        <?php if (empty($transactions)): ?>
                            <tr><td colspan="5" class="px-8 py-8 text-center text-xs font-bold text-[--text-main]/40 italic uppercase">No recent transactions found</td></tr>
                        <?php else: ?>
                            <?php foreach ($transactions as $trx): ?>
                            <tr class="hover:bg-white/[0.01] transition-colors">
                                <td class="px-8 py-5">
                                    <p class="text-sm font-bold text-[--text-main]"><?= htmlspecialchars($trx['gym_name']) ?></p>
                                    <p class="text-[9px] text-[--text-main]/40 font-black uppercase italic tracking-widest">ID: <?= htmlspecialchars($trx['client_subscription_id']) ?></p>
                                </td>
                                <td class="px-8 py-5 text-[10px] font-black text-[--text-main] uppercase italic"><?= htmlspecialchars($trx['plan_name']) ?></td>
                                <td class="px-8 py-5 text-sm font-black text-primary">₱<?= number_format($trx['price'], 2) ?></td>
                                <td class="px-8 py-5 text-[10px] text-[--text-main]/40 font-bold uppercase"><?= date('M d, Y', strtotime($trx['created_at'])) ?></td>
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: 'rgba(<?= hexToRgb($configs['theme_color'] ?? '#8c2bee') ?>, 0.05)',
                borderWidth: 4,
                fill: true,
                tension: 0.4,
                pointRadius: 4,
        ...
        options: {
            ...
            plugins: { 
                ...
                tooltip: {
                    ...
                    titleColor: '<?= $configs['theme_color'] ?? '#8c2bee' ?>',
                    bodyColor: '#fff',
